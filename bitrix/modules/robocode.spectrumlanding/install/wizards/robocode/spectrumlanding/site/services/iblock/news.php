...
if($iblockID == false)
{
	...
	WizardServices::IncludeServiceLang("news.php");
   
	$arrPropertiesTab = array(
		array("edit1", GetMessage("NEWS_TAB_NAME")),	
		array("ACTIVE", GetMessage("NEWS_FIELD_ACTIVE")),
		array("NAME", GetMessage("NEWS_FIELD_NAME")),
	 );

	$arrPropertiesTab = array_merge($arrPropertiesTab, $arAllProperties);	
	
	$afterPropFields = array(
			array("PREVIEW_PICTURE", GetMessage("NEWS_FIELD_PREVIEW_PICTURE")),
			array("PREVIEW_TEXT", GetMessage("NEWS_FIELD_PREVIEW_TEXT")),
	);
