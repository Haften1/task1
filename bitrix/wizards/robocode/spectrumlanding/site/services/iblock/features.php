...
$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH."/xml/".LANGUAGE_ID."/features.xml"; 
$iblockCode = "features_".WIZARD_SITE_ID; 
...
if($iblockID == false)
{
	...
	$arrPropertiesTab = array(
		array("edit1", GetMessage("FEATURES_TAB_NAME")),	
		array("ACTIVE", GetMessage("FEATURES_FIELD_ACTIVE")),
		array("NAME", GetMessage("FEATURES_FIELD_NAME")),
	 );

	$arrPropertiesTab = array_merge($arrPropertiesTab, $arAllProperties);	
	
	$afterPropFields = array(
			array("PREVIEW_PICTURE", GetMessage("FEATURES_FIELD_PREVIEW_PICTURE")),
			array("PREVIEW_TEXT", GetMessage("FEATURES_FIELD_PREVIEW_TEXT")),
	);
...
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/_index.php", array("FEATURES_IBLOCK_ID" => $iblockID));
?>